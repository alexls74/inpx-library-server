...
/**
 * Размер файла для карточки книги: 755 КБ / 13.3 МБ.
 * До 1 МБ — целые килобайты, дальше — один знак после точки.
 * Дробный разделитель намеренно точка во всех локалях: toLocaleString дал бы
 * в русской «13,3», а рядом в той же строке идёт запятая между полями.
 */
export function formatFileSize(bytes) {
  const n = Number(bytes);
  if (!Number.isFinite(n) || n <= 0) return '';
  if (n < 1024) return `${n} ${t('common.unitB')}`;
  if (n < 1024 * 1024) return `${Math.round(n / 1024)} ${t('common.unitKB')}`;
  if (n < 1024 * 1024 * 1024) {
    return `${(n / (1024 * 1024)).toFixed(1)} ${t('common.unitMB')}`;
  }
  return `${(n / (1024 * 1024 * 1024)).toFixed(1)} ${t('common.unitGB')}`;
}
